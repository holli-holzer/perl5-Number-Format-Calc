# This Makefile is for the Number::Format::Calc extension to perl.
#
...
#     ABSTRACT => q[Number::Format::Calc]
#     AUTHOR => [q[Markus Holzer (holli.holzer@gmail.com)]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Number::Format::Calc]
#     PREREQ_PM => { Test::Simple=>q[0.44] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Number/Format/Calc.pm]
...
MAKE = make
FULLEXT = Number/Format/Calc
BASEEXT = Calc
PARENT_NAME = Number::Format
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Number/Format/Calc.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Number/Format/Calc.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Number/Format
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Number/Format
...
TO_INST_PM = lib/Number/Format/Calc.pm

PM_TO_BLIB = lib/Number/Format/Calc.pm \
...
DISTNAME = Number-Format-Calc
DISTVNAME = Number-Format-Calc-0.2
...
manifypods : pure_all  \
	lib/Number/Format/Calc.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/Number/Format/Calc.pm $(INST_MAN3DIR)/Number::Format::Calc.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: Number::Format::Calc' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Markus Holzer (holli.holzer@gmail.com)'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Number-Format-Calc' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Number::Format::Calc",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Markus Holzer (holli.holzer@gmail.com)"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Number-Format-Calc",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Number::Format::Calc</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Markus Holzer (holli.holzer@gmail.com)</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Number/Format/Calc.pm blib/lib/Number/Format/Calc.pm 
